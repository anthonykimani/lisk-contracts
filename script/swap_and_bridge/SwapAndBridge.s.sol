...
contract SwapAndBridgeScript is Script {
    ...
    function run() public {
        ...
        vm.startBroadcast(deployerPrivateKey);
        (bool sent,) = address(swapAndBridgeLido).call{ value: 100000000000 wei }("");
        vm.stopBroadcast();
        if (sent == true) {
            console2.log("SwapAndBridge (Lido) successfully bridged!");
        } else {
            console2.log("SwapAndBridge (Lido) failed to bridge!");
        }
